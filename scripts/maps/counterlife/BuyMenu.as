...
final class BuyMenuCVARS
{
	...
	// Bug?: If a player dies, enters spectator mode and is revived (w/ medkit or cp) 
	// m_fLongJump returns "false" but the physkey "shl" still "1".
	bool m_bLongJump( CBasePlayer@ pPlayer )
	{
		KeyValueBuffer@ pPhysics = g_EngineFuncs.GetPhysicsKeyBuffer( pPlayer.edict() );
		return pPhysics.GetValue( "slj" ) == "1";
	}
	...
	HookReturnCode CS16_ClientSay( SayParameters@ pParams )
	{
		...
		if( args.ArgC() == 1 && FirstArgChecker( args ) )
		{
			...
			if( !m_bLongJump( pPlayer ) )
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You need the LongJump Module\n" );
				return HOOK_CONTINUE;
			}

			g_CLMenu.Show( pPlayer );
		}
		else if( args.ArgC() == 3 && FirstArgChecker( args ) )
		{
			...
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] Invalid Item Type\n" );
				return HOOK_CONTINUE;
			}

			if( g_CLMenu.m_Items.length() > 0 )
			{
				for( uint i = 0; i < g_CLMenu.m_Items.length(); i++ )
				{
					if( szItemType + "_" + args.Arg(2).ToLowercase() == g_CLMenu.m_Items[i].EntityName )
					{
						bItemFound = true;
						szItemName = g_CLMenu.m_Items[i].EntityName;
						uiCost = g_CLMenu.m_Items[i].Cost;
						break;
					}
					else
						bItemFound = false;
				...
				if( bItemFound )
				{
					...
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] Not enough money to buy: " + szItemName + " - Cost: $" + uiCost + "\n" );
					}

					else if( pPlayer.HasNamedPlayerItem( szItemName ) !is null )
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You already have this weapon: " + szItemName + "\n" );
					}
					else
					{ 
				...
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, (args.Arg(2) == string_t()) ?  "[CL BUYMENU] Invalid item\n" : "[CL BUYMENU] Invalid item: " + args.Arg(2) + "\n" );
				}
			}
		}
				...
				if( pPlayer.m_hActiveItem.GetEntity() !is null )
				{
					...
					if( g_CLMenu.m_Items.length() > 0 )
					{
						for( uint i = 0; i < g_CLMenu.m_Items.length(); i++ )
						{
							...
							{
								if( szClassname == g_CLMenu.m_Items[i].EntityName )
								{
									bItemFound = true;
									szItemName = g_CLMenu.m_Items[i].EntityName;
									uiCost = g_CLMenu.m_Items[i].Cost;
							...
							else if( "ammo_" + szClassname.Split("_")[1] == g_CLMenu.m_Items[i].EntityName )
							{
								bItemFound = true;
								szItemName = g_CLMenu.m_Items[i].EntityName;
								uiCost = g_CLMenu.m_Items[i].Cost;
					...
					if( bItemFound )
					{
						...
						{
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] Exhaustible weapon already at max ammo\n" );
						}
						else
						{ 
							if( uint(BuyMenu::BuyPoints[PlayerID( pPlayer )]) >= uiCost )
					...
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] Invalid Weapon or Ammo entity\n" );
					}
				}
			}
		}
	...
	void CS16_Buy( const CCommand@ args )
	{
		...
		if( args.ArgC() == 1 )
		{
			if( !m_bLongJump( pPlayer ) )
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CL BUYMENU] You need the LongJump Module\n" );
				return;
			}
		...
		else if( args.ArgC() == 3 )
		{
			...
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CL BUYMENU] Invalid Item Type\n" );
				return;
			}
			...
			if( g_CLMenu.m_Items.length() > 0 )
			{
				for( uint i = 0; i < g_CLMenu.m_Items.length(); i++ )
				{
					if( szItemType + "_" + args.Arg(2) == g_CLMenu.m_Items[i].EntityName )
					{
						bItemFound = true;
						szItemName = g_CLMenu.m_Items[i].EntityName;
						uiCost = g_CLMenu.m_Items[i].Cost;
				...
				if( bItemFound )
				{
					if( uint(BuyMenu::BuyPoints[PlayerID( pPlayer )]) <= 0 )
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CL BUYMENU] Not enough money to buy: " + szItemName + " - Cost: $" + uiCost + "\n" );
					}
					else if( pPlayer.HasNamedPlayerItem( szItemName ) !is null )
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CL BUYMENU] You already have this weapon: " + szItemName + "\n" );
					}
					else
					{ 
						if( uint(BuyMenu::BuyPoints[PlayerID( pPlayer )]) >= uiCost )
				...
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, (args.Arg(2) == string_t()) ?  "[CL BUYMENU] Invalid item\n" : "[CL BUYMENU] Invalid item: " + args.Arg(2) + "\n" );
				}
			}
		}
		else if( args.ArgC() == 2 )
				...
				if( pPlayer.m_hActiveItem.GetEntity() !is null )
				{
					...
					if( bItemFound )
					{
						...
						//Very tedious check to see if the player already has max ammo for a exhaustible weapon
							...
							pPlayer.m_rgAmmo( pPlayer.HasNamedPlayerItem( szClassname ).GetWeaponPtr().m_iPrimaryAmmoType ) == pPlayer.GetMaxAmmo( szClassname ) )
						{
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CL BUYMENU] Exhaustible weapon already at max ammo\n" );
						}
						else
						{ 
							if( uint(BuyMenu::BuyPoints[PlayerID( pPlayer )]) >= uiCost )
					...
					{
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTCONSOLE, "[CL BUYMENU] Invalid Weapon or Ammo entity\n" );
					}
				}
			}
		}
...
{
	BuyMenu::BuyMenuCVARS@ g_CLMenuHooks = @BuyMenu::BuyMenuCVARS();
	return g_CLMenuHooks.CS16_PlayerPostThink( pPlayer );
}
...
{
	BuyMenu::BuyMenuCVARS@ g_CLMenuHooks = @BuyMenu::BuyMenuCVARS();
	return g_CLMenuHooks.CS16_ClientPutInServer( pPlayer );
}
...
{
	BuyMenu::BuyMenuCVARS@ g_CLMenuHooks = @BuyMenu::BuyMenuCVARS();
	return g_CLMenuHooks.CS16_ClientSay( pParams );
}
...
void MoneyInit()
{
	...
	if( szMapname == "counterlife_c00" || szMapname == "counterlife_c01_*" || szMapname == "counterlife_c02_a1" )
...
final class BuyableItem
{
	...
	private BuyMenu::BuyMenuCVARS@ g_CLMenuHooks = @BuyMenu::BuyMenuCVARS();
	...
	void Fill( CBasePlayer@ pPlayer = null )
	{
		FillAmmo( pPlayer );
	}
	...
	{
		if( uint(BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )]) < m_uiCost )
		{
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] Not enough money to buy: " + m_szEntityName + " - Cost: $" +  m_uiCost + "\n" );
			return;
		}
		...
		if( pPlayer.HasNamedPlayerItem( m_szEntityName ) !is null )
		{
			...
			if( !(pPlayer.HasNamedPlayerItem( m_szEntityName ).iFlags() < 0) && pPlayer.HasNamedPlayerItem( m_szEntityName ).iFlags() & ITEM_FLAG_EXHAUSTIBLE != 0 )
			{
				if( pPlayer.GiveAmmo( pPlayer.HasNamedPlayerItem( m_szEntityName ).GetWeaponPtr().m_iDefaultAmmo, m_szEntityName, pPlayer.GetMaxAmmo( m_szEntityName ) ) != -1 )
				{
					...
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You bought ammo for: " + m_szEntityName + "\n" );
				}
				else
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You already have max ammo for this item!\n" );
					return;
				}
			}
			else
			{
				g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You already have that item!\n" );
				return;
			}
			//KernCore end
		}
		...
		if( uint(BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )]) >= m_uiCost )
		{
			int p = int(BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )]);
			BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )] = p - m_uiCost;
			g_CLMenuHooks.ShowPointsSprite( pPlayer );
		...
		{
			g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] Not enough money - Cost: $" + m_uiCost + "\n");
			return;
		}
	}

	private void FillAmmo( CBasePlayer@ pPlayer ) const
	{
		bool bItemFound = false;
		string szItemName;
		uint uiCost;
		...
		if( pPlayer.m_hActiveItem.GetEntity() !is null )
		{
			...
			if( g_CLMenu.m_Items.length() > 0 )
			{
				...
				{
					//Identify the primary ammo entity first
					if( "ammo_" + szClassname.Split("_")[1] == g_CLMenu.m_Items[i].EntityName )
					{
						bItemFound = true;
						szItemName = g_CLMenu.m_Items[i].EntityName;
						uiCost = g_CLMenu.m_Items[i].Cost;
			...
			if( bItemFound )
			{
				float flAmount = ( pPlayer.HasNamedPlayerItem( szClassname ).GetWeaponPtr().iMaxAmmo1() - pPlayer.m_rgAmmo( pPlayer.HasNamedPlayerItem( szClassname ).GetWeaponPtr().m_iPrimaryAmmoType ) ) 
							/ float( pPlayer.HasNamedPlayerItem( szClassname ).GetWeaponPtr().iMaxClip() );

				uint uiAmount = uint( Math.Ceil( flAmount ) );
				uiCost *= uiAmount;

				if( uint(BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )]) <= 0 )
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] Not enough money to buy: " + szItemName + "(" + uiAmount + ") - Cost: $" + uiCost + "\n" );
				}
				else if( uint(BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )]) >= uiCost && pPlayer.m_rgAmmo( pPlayer.HasNamedPlayerItem( szClassname ).GetWeaponPtr().m_iPrimaryAmmoType ) == pPlayer.GetMaxAmmo( szClassname ) )
				{
					g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You already have max ammo for this weapon\n" );
				}
				else
				{
					if( uint(BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )]) >= uiCost )
					{
						for( uint i = 0; i < uiAmount; i++ )
							pPlayer.GiveNamedItem( szItemName );

						BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )] = uint(BuyPoints[g_CLMenuHooks.PlayerID( pPlayer )]) - uiCost;
						g_CLMenuHooks.ShowPointsSprite( pPlayer );
					}
					else
					{
...
final class BuyMenu
{
	...
	//Counter-Life
	private string szFillAmmoOption  	= "$??? Fill Ammo";
	private uint m_uiMethod			= 0; //2:from func_recharge, 1:from func_healthcharger
	...
	void Show( CBasePlayer@ pPlayer = null, uint uiMode = 0 )
	{
		m_uiMethod = uiMode;
	...
	private void CreateMenu()
	{
		...
		m_pAmmoMenu.AddItem( szFillAmmoOption );
	...
	private void MainCallback( CTextMenu@ menu, CBasePlayer@ pPlayer, int iSlot, const CTextMenuItem@ pItem )
	{
		if( pItem !is null )
		{
			...
			{
				m_pSecondMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenTercWeapMenu )
			{
				m_pThirdMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenQuatWeapMenu )
			{
				m_pFourthMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenQuinWeapMenu )
			{
				m_pFifthMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenSenaWeapMenu )
			{
				m_pSixthMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenEquiWeapMenu )
			{
				m_pEquipMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenAmmoWeapMenu )
			{
				m_pAmmoMenu.Open( 5, 0, pPlayer );
			}
		}
	}
	...
	private void MainAmmoCallback( CTextMenu@ menu, CBasePlayer@ pPlayer, int iSlot, const CTextMenuItem@ pItem )
	{
		if( pItem !is null )
		{
			...
			if( sChoice == szOpenSecoWeapMenu )
			{
				m_pSecondAMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenTercWeapMenu )
			{
				m_pThirdAMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenQuatWeapMenu )
			{
				m_pFourthAMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenQuinWeapMenu )
			{
				m_pFifthAMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szOpenSenaWeapMenu )
			{
				m_pSixthAMenu.Open( 5, 0, pPlayer );
			}
			else if( sChoice == szFillAmmoOption )
			{
				BuyableItem("","",0,"").Fill( pPlayer);
			}
		}
	}
	...
	private void EquipCallback( CTextMenu@ menu, CBasePlayer@ pPlayer, int iSlot, const CTextMenuItem@ pItem )
	{
		if( pItem !is null )
		{
			...
			if( pBuyItem !is null )
			{
				if( (pBuyItem.EntityName != ITEM_HEALTHKIT::GetName()) && (pBuyItem.EntityName != ITEM_KEVLAR::GetName()) )
				{
					pBuyItem.Buy( pPlayer );
				...
				else
				{
					if( m_uiMethod == 2 )
					{
						if( pBuyItem.EntityName == ITEM_KEVLAR::GetName() )
							pBuyItem.Buy( pPlayer );
						else
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You can only buy this through medical buystations\n" );
					}
					else if( m_uiMethod == 1 )
					{
						if( pBuyItem.EntityName == ITEM_HEALTHKIT::GetName() )
							pBuyItem.Buy( pPlayer );
						else
							g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You can only buy this through suitcharger buystations\n" );
					}
					else
						g_PlayerFuncs.ClientPrint( pPlayer, HUD_PRINTTALK, "[CL BUYMENU] You can only buy this through buystations\n" );
				}
			}
		}
	}
